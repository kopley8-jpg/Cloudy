    ...
    return (
        <View style={styles.container}>
            <View
                ...
                onTouchEnd={onThumbsContainerMoveComplete}
            >
    ...
    return (
        <View
            ...
            style={{
                ...
                height: "90%",
                borderWidth: 3,
                borderColor: isPicked ? "white" : "grey",
                backgroundColor: stop.color,
                position: "absolute",
                left: `${stop.offset * 100 - 3}%`
            }}
        >
            <Text>{index}</Text>
        </View>
...
export const ColorPathWithLocalStates: React.FC<{ stops: Stop[] }> = ({ stops }) => {
    ...
    const changeOffsetForPickedStop = (offset: number) => {
        ...
        const newStops = [...sortedStops].map((stop) => (stop.id === pickedStopId ? { ...stop, offset } : stop))
        setEditingStops(newStops)
    }

    const changePickedStopId = (id: number) => {
        setPickedStopID((prev) => (prev === id ? null : id))
    }
    ...
    return (
        <View style={styles.container}>
            ...
            <ColorPathPreview stops={editingStops} />
        </View>
    )
}
